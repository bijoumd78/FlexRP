...
{
    spdlog::info("{}({}), in_avail = {}, size = {}, max_size = {}.", __FUNCTION__, t_bytesTransferred, m_requestBuf_.in_avail(), m_requestBuf_.size(), m_requestBuf_.max_size());
...
void Session::readData(std::istream &stream)
{
    ...
    spdlog::info("{} size is {}, tellg = {}", m_fileName, m_fileSize, stream.tellg());
}
...
{
    spdlog::info("{} in {} due to {}", __FUNCTION__, t_functionName, t_ec.message() );
}


Server::Server(IoService& t_ioService, short t_port, std::string const& t_workDirectory)
    : m_socket(t_ioService),
      m_acceptor(t_ioService, boost::asio::ip::tcp::endpoint(boost::asio::ip::tcp::v4(), static_cast<unsigned short>(t_port))),
