...
using shared_string =
    bip::basic_string<char, std::char_traits<char>, char_alloc>;
using ring_buffer =
    boost::lockfree::spsc_queue<shared_string, boost::lockfree::capacity<200> >;

void FlexRPSharedMemory::createSharedMemory(const std::vector<std::string> &v) {
  bip::managed_shared_memory segment(bip::open_or_create, "FlexRPSharedMemory",
                                     65536);
  ...
  for (const auto &e : v) queue->push(shared_string(e.c_str(), char_alloc));
}

void FlexRPSharedMemory::getReconmoduleProperty(std::vector<std::string> &v,
                                                const size_t n) {
  bip::managed_shared_memory segment(bip::open_or_create, "FlexRPSharedMemory",
                                     65536);
  char_alloc char_alloc(segment.get_segment_manager());
